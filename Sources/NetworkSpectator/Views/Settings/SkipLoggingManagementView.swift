...
struct SkipLoggingManagementView: View {
    ...
    @State private var skipLogging: [LoggingExclusionRule] = []
    ...
    private func skipLoggingItemRow(_ item: LoggingExclusionRule) -> some View {
        HStack(spacing: 12) {
        ...
        .onTapGesture {
            if let skipRequest = LoggingExclusionManager.shared.skipRequests.first(where: { $0.id == item.id }),
    ...
    private func loadData() {
        withAnimation {
            skipLogging = LoggingExclusionManager.shared.skipRequests.map { $0 }
        }
        onDataChanged?()
    }
